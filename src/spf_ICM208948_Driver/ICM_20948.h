...
public:
    ...
    // Enable debug messages using the chosen Serial port (Stream)
  	...
  	#else
  	void enableDebugging(Stream &debugPort); //Given a port to print to, enable debug messages.
  	#endif
  	#else
  	void enableDebugging(Stream &debugPort); //Given a port to print to, enable debug messages.
  	#endif
    ...
    ICM_20948_AGMT_t getAGMT(bool get_settings=false); // Updates the agmt field in the object and also returns a copy directly
    ICM_20948_AGMT_t getAG(bool get_settings=false); // Updates the ag field in the object and also returns a copy directly
...
public:
    ...
    virtual ICM_20948_Status_e begin(I2C &i2c, bool ad0val = true, uint8_t ad0pin = ICM_20948_ARD_UNUSED_PIN);
};
